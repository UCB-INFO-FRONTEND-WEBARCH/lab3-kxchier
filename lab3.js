...
const analyticsData = [
  ...
  const findLongestSessionUser = (data) => {
    ...
    for (const user of data) {
      if (user.avgSessionDuration > max_duration) {
        max_duration = user.avgSessionDuration;
        max_session_user = user.name;
      }
    }

    return max_session_user;
  };
  ...
  const getTotalSessions = (data) => {
    ...
    return data.reduce((prev, curr) => prev + curr.totalSessions, 0);
  };
